...
const Home = () => {
  ...
  return (
    <main className='m-4 md:mx-auto  md:w-11/12'>
      <h3 className='mb-4 w-max bg-[#3B0D11] px-4 py-2 text-lg font-semibold text-white sm:text-xl md:ml-4 md:px-6  md:py-3 lg:px-8 lg:text-2xl'>
        Trending
      </h3>
      <PrincipalCard
      ...
      <section className=' grid grid-cols-auto-fit md:border-y md:border-y-black/25'>
        {news.map(({ id, title, description, image, url }, index) => (
          <NewsCard
            key={index}
            title={title}
